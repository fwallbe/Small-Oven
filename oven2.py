...
# Which sensor drives the controller PV (0,1,2) (NOTE: controller below uses AVG by default)
...
# ---- PV prediction (lead compensation) ----
# Uses: pv_pred = pv + tau * d(pv)/dt (with filtered slope)
PREDICT_ENABLE = True
PREDICT_TAU_S = 12.0              # seconds of look-ahead (start 5..20 for ovens)
PREDICT_SLOPE_LP_TAU_S = 3.0      # slope low-pass tau (s)
PREDICT_CLAMP_C = 10.0            # max |correction| added to pv (°C)
...
class PvPredictor:
    """
    Lead prediction: pv_pred = pv + tau * d(pv)/dt (with low-pass filtered slope).
    """
    def __init__(self, tau_s: float, slope_lp_tau_s: float, clamp_c: float):
        self.tau_s = float(tau_s)
        self.slope_lp_tau_s = float(slope_lp_tau_s)
        self.clamp_c = float(clamp_c)
    ...
    def update(self, pv: float, t_now: float) -> tuple[float, float]:
        if self.prev_t is None or self.prev_pv is None or math.isnan(pv):
            self.prev_t = t_now
            self.prev_pv = pv
            self.slope_t = t_now
            return pv, 0.0
        ...
        slope = (pv - self.prev_pv) / dt  # °C/s

        if self.slope_t is None:
            self.slope_t = t_now
        dt_s = max(1e-6, t_now - self.slope_t)
        a = lp_alpha(dt_s, self.slope_lp_tau_s)
        self.slope_f = self.slope_f + a * (slope - self.slope_f)
        self.slope_t = t_now

        corr = self.tau_s * self.slope_f
        corr = clamp(corr, -self.clamp_c, self.clamp_c)
        pv_pred = pv + corr

        self.prev_t = t_now
        self.prev_pv = pv
        return pv_pred, self.slope_f
...
class ControlThread(QThread):
    ...
    def __init__(self, profile: Profile, initial_target_c: float, parent=None):
        ...
        self.pred = PvPredictor(
            tau_s=PREDICT_TAU_S,
            slope_lp_tau_s=PREDICT_SLOPE_LP_TAU_S,
            clamp_c=PREDICT_CLAMP_C
        )
    ...
    def run(self):
        ...
        try:
            ...
            while self._running and not reached_initial:
                ...
                pv_pred, slope_f = (pv, 0.0)
                pv_for_pid = pv
                if PREDICT_ENABLE:
                    pv_pred, slope_f = self.pred.update(pv, loop_start)
                    pv_for_pid = pv_pred

                if self._force_off:
                    duty = 0.0
                else:
                    duty = self.pid.update(setpoint=self.initial_target_c, measurement=pv_for_pid, t_now=loop_start)
                ...
                self.update.emit({
                    ...
                    "actual_c": pv,
                    "pv_pred_c": pv_pred,
                    "slope_c_per_s": slope_f,
                    "temps_c": temps,
                    "forced_off": self._force_off,
                    "duty": duty,
                    "done": False,
                    "note": f"{self.cal.note} (PID uses {'PVpred' if PREDICT_ENABLE else 'PV'}; PV=AVG sensors)"
                })
            ...
            while self._running:
                ...
                if math.isnan(pv):
                    self.force_heater_off()
                    self.fault.emit("Lost temperature data (Average is NaN) — heater forced OFF")
                    break

                if pv > TEMP_MAX_C or pv < TEMP_MIN_C:
                    self.force_heater_off()
                    self.fault.emit(f"Average temp out of bounds: {pv:.2f} °C (heater forced OFF)")
                    break
                ...
                if self._force_off:
                    ...
                    duty = self.pid.update(setpoint=sp, measurement=pv_for_pid, t_now=loop_start)
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        pv_mode = "PVpred(avg)" if PREDICT_ENABLE else "PV(avg)"
        self.lbl_status = QLabel(
            f"Status: idle | heaterGPIO={HEATER_GPIO_BCM} | fansGPIO={FANS_GPIO_BCM} | loop={CONTROL_LOOP_HZ}Hz | PWM={HEATER_PWM_HZ}Hz"
            f" | cal={CALIBRATION_CSV_PATH} | LPF tau={TEMP_LP_TAU_S}s | PID-meas={pv_mode}"
        )
        self.lbl_now = QLabel("stage=-- | t=-- min | SP=-- °C | PV=-- °C | PVpred=-- °C | dT/dt=-- °C/s | avg=-- °C | T0/T1/T2=--/--/-- | heater=-- | duty=--")
        ...
        self.curve_pv = self.plot.plot([], [], name="PV (avg)", pen=pg.mkPen(width=1))
        self.curve_pvpred = self.plot.plot([], [], name="PV predicted", pen=pg.mkPen(width=1))
    ...
    def on_update(self, d: dict):
        ...
        pv_pred = float(d.get("pv_pred_c", float("nan")))
        slope = float(d.get("slope_c_per_s", float("nan")))
        ...
        self.hist_pvpred.append(pv_pred)
        ...
        self.curve_pvpred.setData(self.hist_t_min, self.hist_pvpred)
        ...
        self.lbl_now.setText(
            f"stage={stage} | t={t_min:>6.2f} min | SP={sp:>6.1f} °C | PV={pv:>6.2f} °C"
            f" | PVpred={pv_pred:>6.2f} °C | dT/dt={slope:>7.4f} °C/s"
            f" | avg={avg:>6.2f} °C | T0/T1/T2={t0:>6.2f}/{t1:>6.2f}/{t2:>6.2f}"
            f" | heater={heater_txt} | duty={duty:0.2f}{note_txt}"
        )
